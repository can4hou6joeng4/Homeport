---
title: Unicode braille is an 8-pixel bitmap font
slug: unicode-braille-8-pixel-bitmap-font
date: 2026-09-14
lang: en
description: Each braille character is a 2×4 dot bitmap. Threshold it and you get hard edges; run Floyd–Steinberg error diffusion over the dot grid first and a photo survives as plain text. How the pipeline in a browser image-to-ASCII converter works, with the real code and before/after figures.
image: images/braille-2-threshold-vs-dither.webp
summary_zh: 盲文字符其实是 2×4 的点阵位图。在点阵网格上先做 Floyd–Steinberg 误差扩散再打包，照片就能以纯文本形式存活。附引擎源码与前后对比图。
summary_en: A braille character is secretly a 2×4 bitmap. Dither the dot grid before packing and a photo survives as plain text. With the engine's own code and before/after figures.
---

*Written against [`src/ascii-engine.ts`](https://github.com/can4hou6joeng4/Semaphore/blob/main/src/ascii-engine.ts) as of commit 46a5f63 (the file's last change)
...
page](https://semaphore.bobochang.cn/charsets/braille) renders a live example of the pipeline above.*
